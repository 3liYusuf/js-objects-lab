...
*/
game.party.push(pokemon[150], pokemon[148], pokemon[142]);
...
pokemon.forEach((poky) => {
    poky.starter ? console.log(poky.name) : null;
})
...
game.gymStatus = function () {
    ...
    game.gyms.forEach((poky) => {
        poky.completed ? gymTally.completed++ : gymTally.incomplete++;
    })
...
game.catchPokemon = function (pokemonName) {
    const pokemonObj = pokemon.find(({ name }) => name.toLowerCase() === pokemonName.toLowerCase());
...
*/
const categorizedPokemon = {};

pokemon.forEach((poky) => {
    if (!categorizedPokemon[poky.type]) {
        categorizedPokemon[poky.type] = [];
    }

    categorizedPokemon[poky.type].push(poky);
});

console.log(categorizedPokemon);
